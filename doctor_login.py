    ...
    def forget_password(self):
        ...
        if self.cmb_question.get()=="Select" or self.txt_answer.get()=="" or self.txt_password1.get()=="":
            ...
            try:
                ...
                cur=con.cursor()
                cur.execute("select * from doctor where email=%s and question=%s and answer=%s",(self.txt_email.get(),self.cmb_question.get(),self.txt_answer.get()))
                row=cur.fetchone()
                if row==None:
                   messagebox.showerror("Error","Please Select the correct Security Question / Enter Answer", parent=self.root2) 
                   
                else:
                    cur.execute("update doctor set password=%s where email=%s",(self.txt_password1.get(),self.txt_email.get()))
                    con.commit()
                    con.close()
    ...
    def login(self):
        if self.txt_email.get()=="" or self.txt_password.get()=="":
            ...
            try:
                con=pymysql.connect(host="localhost",user="root",password="",database="management")
                cur=con.cursor()
                cur.execute("select * from doctor where email=%s and password=%s",(self.txt_email.get(),self.txt_password.get()))
                row=cur.fetchone()
                if row==None:
